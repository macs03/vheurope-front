...
angular
    ...
    function SuccessController (utilityService, $scope, $rootScope, $interval, $stateParams,$location,apiUrl,sessionStorageService,$analytics) {
        ...
        $analytics.eventTrack('# Passages Departure', {  category: 'Success', label: 'Number of tickets purchased', value: successData.departureData[0].seats.length });
        $analytics.eventTrack('Bus Company Departure', {  category: 'Success', label: successData.departureData[0].companyName });
        $analytics.eventTrack('Trip Duration Departure', {  category: 'Success', label: successData.departureData[0].duration });
        $analytics.eventTrack('Type Service Departure', {  category: 'Success', label: successData.departureData[0].typeService });
        ...
        if(typeof(successData.returnData[0])== 'object'){
            ...
            $analytics.eventTrack('Trip Duration Return', {  category: 'Success', label: successData.returnData[0].duration });
            $analytics.eventTrack('Type Service Return', {  category: 'Success', label: successData.returnData[0].typeService });
